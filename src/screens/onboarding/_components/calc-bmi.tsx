...
export const BMIDisplay = ({ bmi }: BMIDisplayProps) => {
  ...
  const validBmi = typeof bmi === 'number' && isFinite(bmi) ? Math.max(0, bmi) : 0;
  const progressPercentage = isFinite(validBmi) ? Math.min((validBmi / 40) * 100, 100) : 0;
  ...
  return (
    <Card size="lg" className="gap-6">
      ...
      <Box className="items-baseline gap-3">
        ...
        <Text className={`text-xl font-semibold ${getBMICategoryColor()}`}>{bmiCategory}</Text>
      </Box>
